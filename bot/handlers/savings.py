...
async def start_savings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["savings_action"] = "set"
    context.user_data["savings_prompt_chat_id"] = update.effective_chat.id
    context.user_data["savings_prompt_message_id"] = (
...
async def start_adjustment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["savings_action"] = "adjust"
    context.user_data["savings_prompt_chat_id"] = update.effective_chat.id
    context.user_data["savings_prompt_message_id"] = (
        update.callback_query.message.message_id
    )
...
async def set_savings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not saved:
        await send_message(
            update,
            "❌ Starting savings could not be saved",
        )
        clear_savings_data(context)
        return ConversationHandler.END

    await finish_savings_prompt(
        context,
        f"✅ Starting savings set to {amount:.2f} AMD",
    )
...
async def set_adjustment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not saved:
        await send_message(
            update,
            "❌ Balance adjustment could not be saved",
        )
        clear_savings_data(context)
        return ConversationHandler.END
    ...
    await finish_savings_prompt(
        context,
        f"✅ Balance adjusted by {sign}{amount:.2f} AMD",
    )
...
async def handle_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("savings_action") == "adjust":
        return await set_adjustment(update, context)

    return await set_savings(update, context)
...
async def finish_savings_prompt(
...
def clear_savings_data(context: ContextTypes.DEFAULT_TYPE):
    for key in (
        "savings_prompt_chat_id",
        "savings_prompt_message_id",
        "savings_action",
    ):
